...
public class ProjectileFactory {
	...
	public static Projectile createNormalProjectile(Entity owner, double collisionRadius, BufferedImage texture) {
		...
		actionList.add(new Action(Player.class, entity -> {
			...
			player.getStatManager().setHealth(player.getStatManager().getCurrentHealth() - 
					Math.round((newProjectile.physicalDmg - newProjectile.magicDmg) * (1 - player.getEquipment().calculateTotalArmorValue()/100)));
